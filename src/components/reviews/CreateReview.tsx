...
import { Divide, Lock, Star } from "lucide-react";
      ...
      {!user ? (
        ...
        >
          ...
          <div className="flex gap-1 mb-3">
            {Array.from({ length: 5 }).map((_, index) => (
              <Star
                key={index}
                className={cn(
                  "size-8 lg:size-7 cursor-pointer hover:scale-110 transition-all"
                )}
          ...
          <div className="w-full sm:w-[65%] lg:w-[50%] flex flex-col gap-2">
            {state && !state.ok ? (
              <p className="text-[1.1em]">{state?.text}</p>
            ) : (
            ...
            ></textarea>
            <button className="w-full text-xl py-2 flex justify-center bg-gradient-to-br from-[#5f9beb]/90 to-[#5f9beb]/60 hover:opacity-95 active:opacity-85 rounded-xs cursor-pointer transition-opacity">
              Post
            </button>
          </div>
        </form>
